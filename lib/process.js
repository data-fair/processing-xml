...
module.exports = async (processingConfig, tmpDir, axios, log) => {
  ...
  const separateur = processingConfig.separateur || '/'
  const tab = []
  ...
  for (const key in obj) {
    if (key !== 'Val' || nb === 1) {
      for (const key2 in obj[key]) {
        obj[key][key2] = flatten(obj[key][key2], separateur)
  ...
  if (processingConfig.datasetMode === 'create' || processingConfig.datasetMode === 'update') {
    ...
    const newHeader = lines[0].replace(/\./g, separateur)
    lines[0] = newHeader
    ...
    await log.step('Mettre à jour des lignes d\'un jeu de données (incrémental)')
    for (let i = 0; i < tab.length; i++) {
      tab[i] = flattenObject(tab[i], '', separateur)
    }

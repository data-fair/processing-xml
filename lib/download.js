...
exports.download = async (processingConfig, dir = 'data', axios, log) => {
  ...
  const fileName = path.parse(new URL(processingConfig.url).pathname).name + '.' + extension
  ...
  if (extension === 'zip') {
    ...
    try {
      await execFile('unzip', ['-o', file, '-d', dir])
      const files = await fs.readdir(dir)
      const xmlFiles = files.filter(f => f.endsWith('.xml'))
      const targetName = `${path.parse(fileName).name}.xml`
      for (const xmlFile of xmlFiles) {
        if (xmlFile !== targetName) {
          await fs.move(path.join(dir, xmlFile), path.join(dir, targetName), { overwrite: true })
          break
        }
      }
    } catch (err) {
      await log.warning('Impossible d\'extraire l\'archive, le fichier est peut-être déjà extrait')
    }
    // remove the zip file
